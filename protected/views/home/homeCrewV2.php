<?php 
 ...
 $trip_id = isset($data['data']['post']['post']['trip_id']) ? $data['data']['post']['post']['trip_id'] : null;

$this->widget('ext.dropDownChain.VDropDownChain', array(
    'parentId' => 'Booking_asal',
    'secondParentId' => 'Booking_trip_id',
    'childId' => 'Booking_tujuan',
    'url' => 'api/getTujuanKota?id=h3n5r5w5q584g4r4a4a356g4m5i484b4o4e4t5p5u5t4e4w2',
    'valueField' => 'drof_off_city',
    'textField' => 'tujuan',
));
?>
<style>
...
<div class="row mt-10">
    <div class="col-md-12 col-sm-12 col-xs-12">
      ...
      <div class="row d-relative">
        <div class="col-md-12 col-sm-12 col-xs-12">
              <?= CHtml::dropDownList('Booking[asal]', isset($arrayTujuan['dataSelected']['boarding_city']) ? $arrayTujuan['dataSelected']['boarding_city'] : '', isset($arrayTujuan['naik']) ? $arrayTujuan['naik'] : [], 
              ['class' => 'form-control','prompt'=>'Pilih Asal','required'=>true]); ?>
        </div>
      </div>
      <div class="row d-relative">
        <div class="col-md-12 col-sm-12 col-xs-12">
              <?= CHtml::hiddenField('Booking[tujuan_id]', $model->tujuan); ?>
              <?= CHtml::hiddenField('Booking[trip_id]', $model->trip_id); ?>
              <?= CHtml::dropDownList('Booking[tujuan]', isset($arrayTujuan['dataSelected']['drop_off_city']) ? $arrayTujuan['dataSelected']['drop_off_city'] : '', isset($arrayTujuan['turun']) ? $arrayTujuan['turun'] : [], 
    ...
    $this->widget('zii.widgets.CListView', array(
        ...
        'itemView'=>'_view_list_booking_v2',
        'emptyText' => (isset($penugasan['message']) && !empty($penugasan['message']) ? $penugasan['message'] : 'Silahkan pilih tujuan terlebih dahulu'),
    )); 
    
    
    echo $this->renderPartial('_bottom_field_v2', array(
        'data' => $data
        ), false); ?>
</div>
...
<script>
    ...
    <?php if (isset($trip_id) && !empty($trip_id)): ?>
...
function refreshListBooking(latitude, longitude) {
    ...
    if (latitude !== null && longitude !== null) {
        ...
        location.href="<?= Constant::baseUrl().'/'.$this->route.'?startdate=' ?>"+$('#Booking_startdate').val()+"&latitude=-7.8033209&longitude=110.312795&rit="+$('#Booking_rit').val()+"&tujuan="+$('#Booking_tujuan_id').val()+"&naik="+$('#Booking_asal').val()+"&turun="+$('#Booking_tujuan').val();
    }
}
...
var today = new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate());
  $('#Booking_startdate').datepicker({
    ...
    function searchTujuan(asal, tujuan, trip_id)
    {
        ...
        $.ajax({
              type : "POST",
              url : "<?= Constant::baseUrl() . '/booking/searchTujuan' ?>",
              dataType : "JSON",
              data: dataSend,
              success : function(data) {
                if (typeof data.data.tujuan_id !== "undefined" && data.data.tujuan_id !== "") {
