...
class CUploadedFile extends CComponent
{
	 ...
	 */
	public static function getInstances($model, $attribute, $loop)
	{
		return self::getInstancesByName(CHtml::resolveName($model, $attribute), $loop);
	}
	 ...
	 */
	public static function getInstancesByName($name, $loop)
	{
		if(null===self::$_files)
			self::prefetchFiles();
		...
		$len=strlen($name);
		$results=null;
		foreach(array_keys(self::$_files) as $index => $key){
			// if(0===strncmp($key, $name.'[', $len+1) && self::$_files[$key]->getError()!=UPLOAD_ERR_NO_FILE)
			if ($loop == $index){
				if(isset(self::$_files[$key]) && self::$_files[$key]->getError()!=UPLOAD_ERR_NO_FILE)
